...
function generateHtml(storyPath, theme, t) {
    // Get text up to the first newline for the title.
    let titleText = theme.split('\n')[0];
    // Truncate if it's still over 50 characters.
    const title = titleText.length > 50 ? escapeHtml(titleText.substring(0, 50)) + '...' : escapeHtml(titleText);
    ...
    <div class="story-container"><h1>${title}</h1><p class="theme">${escapeHtml(theme).replace(/\n/g, '<br>')}</p>`;
    
    for (let i = 0; i < storyPath.length; i++) {
        const node = storyPath[i];
        const nextNode = storyPath[i + 1];
        ...
        const choiceText = nextNode && nextNode.choiceText 
            ? `<p class="choice-text"><strong>${escapeHtml(t.historyChoicePrefix)}:</strong> "${escapeHtml(nextNode.choiceText)}"</p>` 
            : '';
